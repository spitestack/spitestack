...
import { SegmentIndex } from '../../../src/storage/segment-index';
...
describe('SegmentIndex', () => {
  ...
  describe('addBatch', () => {
    test('should add events to stream index', () => {
      const events = [
        createEvent({ streamId: 'stream-1', revision: 0, globalPosition: 0n }),
        createEvent({ streamId: 'stream-1', revision: 1, globalPosition: 1n }),
      ];
    ...
    test('should add events to position index', () => {
      const events = [
        createEvent({ streamId: 'stream-1', revision: 0, globalPosition: 42n }),
      ];
  ...
  describe('findByStream', () => {
    beforeEach(() => {
      ...
      index.addBatch(1n, 100, [
        createEvent({ streamId: 'user-123', revision: 0, globalPosition: 0n }),
        createEvent({ streamId: 'user-123', revision: 1, globalPosition: 1n }),
      ]);
      index.addBatch(1n, 200, [
        createEvent({ streamId: 'user-123', revision: 2, globalPosition: 2n }),
        createEvent({ streamId: 'user-456', revision: 0, globalPosition: 3n }),
      ]);
      index.addBatch(2n, 100, [
        createEvent({ streamId: 'user-123', revision: 3, globalPosition: 4n }),
      ]);
    });
  ...
  describe('findByPosition', () => {
    test('should find event by global position', () => {
      index.addBatch(1n, 100, [
        createEvent({ streamId: 'stream-1', globalPosition: 42n }),
      ]);
    ...
    test('should return undefined for unknown position', () => {
      const location = index.findByPosition(999n);
  ...
  describe('evictSegment', () => {
    beforeEach(() => {
      ...
      index.addBatch(2n, 100, [
        createEvent({ streamId: 'stream-1', revision: 2, globalPosition: 2n }),
        createEvent({ streamId: 'stream-2', revision: 0, globalPosition: 3n }),
      ]);
    });
    ...
    test('should remove entries from position index', () => {
      expect(index.findByPosition(0n)).toBeDefined();
      expect(index.findByPosition(2n)).toBeDefined();

      index.evictSegment(1n);

      expect(index.findByPosition(0n)).toBeUndefined();
      expect(index.findByPosition(1n)).toBeUndefined();
      expect(index.findByPosition(2n)).toBeDefined();
    });
  ...
  describe('clear', () => {
    test('should remove all data', () => {
      index.addBatch(1n, 100, [
        createEvent({ streamId: 'stream-1', globalPosition: 0n }),
      ]);
  ...
  describe('rebuildFromSegment', () => {
    ...
    test('should rebuild index from segment file', async () => {
      ...
      await writer.appendBatch([
        createEvent({ streamId: 'stream-2', revision: 0, globalPosition: 2n }),
      ]);
      await writer.sync();
      await writer.close();
    ...
    test('should evict existing entries before rebuilding', async () => {
      ...
      index.addBatch(1n, 999, [
        createEvent({ streamId: 'old-stream', revision: 0, globalPosition: 999n }),
      ]);

      // Write new segment
      await writer.open('/data/segment.log', 1n, 0n);
      await writer.appendBatch([
        createEvent({ streamId: 'new-stream', revision: 0, globalPosition: 0n }),
      ]);
      await writer.sync();
      await writer.close();
      ...
      expect(index.findByPosition(999n)).toBeUndefined();
    });
  });
});
