...
import { SegmentManager } from '../../../src/storage/segment-manager';
...
import { SEGMENT_HEADER_SIZE } from '../../../src/storage/segment-header';
...
describe('SegmentManager', () => {
  ...
  describe('initialize', () => {
    ...
    test('should scan existing segments', async () => {
      ...
      await manager1.writeBatch([createEvent({ globalPosition: 0n })]);
      await manager1.sync();
    ...
    test('should track global position from existing segments', async () => {
      ...
      await manager1.writeBatch([
        createEvent({ globalPosition: 0n }),
        createEvent({ globalPosition: 1n }),
        createEvent({ globalPosition: 2n }),
      ]);
      await manager1.sync();
      await manager1.close();
      ...
      expect(manager2.getGlobalPosition()).toBe(3n);
  ...
  describe('writeBatch', () => {
    ...
    test('should track global position from events', async () => {
      ...
      await manager.writeBatch([
        createEvent({ globalPosition: 100n }),
        createEvent({ globalPosition: 101n }),
      ]);

      expect(manager.getGlobalPosition()).toBe(102n);
  ...
  describe('allocateGlobalPosition', () => {
    test('should return incrementing positions', async () => {
      ...
      expect(manager.allocateGlobalPosition()).toBe(0n);
      expect(manager.allocateGlobalPosition()).toBe(1n);
      expect(manager.allocateGlobalPosition()).toBe(2n);
