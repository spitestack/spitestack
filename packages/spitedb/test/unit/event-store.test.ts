...
describe('EventStore', () => {
  ...
  describe('append', () => {
    ...
    test('should increment global position', async () => {
      ...
      expect(r1.globalPosition).toBe(0n);
      expect(r2.globalPosition).toBe(1n);
      expect(r3.globalPosition).toBe(2n);
    });
  ...
  describe('readGlobal', () => {
    ...
    test('should filter by fromPosition', async () => {
      const events = await store.readGlobal(1n);

      expect(events).toHaveLength(2);
      expect(events[0]!.globalPosition).toBe(1n);
    });

    test('should limit with maxCount', async () => {
      const events = await store.readGlobal(0n, { maxCount: 2 });
