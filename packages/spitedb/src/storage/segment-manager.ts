...
export class SegmentManager {
  ...
  constructor(
    ...
    this.reader = new SegmentReader(fs, serializer, compressor);
        ...
        for (const streamId of indexFile.getAllStreamIds()) {
          ...
          for (const entry of entries) {
            if (entry.globalPosition >= this.globalPosition) {
              this.globalPosition = entry.globalPosition + 1n;
            }
          }
        }
      } catch (error) {
        // Log and skip segments that can't be loaded
   ...
   */
  async getWriter(): Promise<SegmentWriter> {
  ...
  async rotateSegment(): Promise<void> {
    ...
    await this.activeWriter.open(path, segmentId, this.globalPosition);
  ...
  /**
   * Write a batch of events to the active segment.
   *
   * Convenience method that handles writer acquisition and size tracking.
   *
   * @returns Write result with position info
   */
  async writeBatch(events: StoredEvent[]): Promise<BatchWriteResult> {
    ...
    for (const event of events) {
      if (event.globalPosition >= this.globalPosition) {
        this.globalPosition = event.globalPosition + 1n;
      }
