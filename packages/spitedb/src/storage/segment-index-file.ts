...
export class SegmentIndexFile {
  ...
  private parseEntries(): void {
    ...
    for (let i = 0; i < this.header.entryCount; i++) {
      ...
      const globalPosition = view.getBigUint64(entryOffset + 8, true);
  ...
  findByStream(
    ...
    while (left <= right) {
      ...
      const cmp = entry.streamId.localeCompare(streamId);
   ...
   */
  findByGlobalPosition(position: bigint): IndexEntry | undefined {
  ...
  static async write(
    ...
    // Sort entries by (streamId, revision)
    const sorted = [...entries].sort((a, b) => {
      const cmp = a.streamId.localeCompare(b.streamId);
      if (cmp !== 0) return cmp;
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      view.setBigUint64(entryOffset + 8, entry.globalPosition, true);
