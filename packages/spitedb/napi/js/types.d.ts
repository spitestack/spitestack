...
export type SchemaToRow<TSchema extends SchemaDefinition> = {
    [K in keyof TSchema as K extends PrimaryKeyColumns<TSchema> ? never : K]: TSchema[K] extends ColumnType ? ColumnTypeToTS<TSchema[K]> : TSchema[K] extends {
        type: ColumnType;
    } ? ColumnTypeToTS<TSchema[K]['type']> : unknown;
} & {
    [K in PrimaryKeyColumns<TSchema>]?: TSchema[K] extends {
        type: ColumnType;
    } ? ColumnTypeToTS<TSchema[K]['type']> : unknown;
};
/**
 ...
 */
export type ColumnTypeToTS<T extends ColumnType> = T extends 'text' ? string : T extends 'integer' ? number : T extends 'real' ? number : T extends 'boolean' ? boolean : T extends 'blob' ? Buffer : unknown;
/**
