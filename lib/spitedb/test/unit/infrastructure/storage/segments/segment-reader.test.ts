...
import { SEGMENT_HEADER_SIZE, SEGMENT_MAGIC } from '../../../../../src/infrastructure/storage/segments/segment-header';
...
describe('SegmentReader', () => {
  ...
  describe('readHeader', () => {
    test('should read valid segment header', async () => {
      ...
      expect(header.basePosition).toBe(0);
    });

    test('should read header with custom segment ID and base position', async () => {
      await writer.open('/data/segment.log', 42n, 1000);
      ...
      expect(header.basePosition).toBe(1000);
    });
  ...
  describe('integration', () => {
    test('should round-trip complex events', async () => {
      ...
      expect(readEvents[0]!.globalPosition).toBe(100);
      ...
      expect(readEvents[1]!.globalPosition).toBe(101);
    });
  });
});
