...
import {
  ...
  INDEX_ENTRY_SIZE,
  type IndexEntry,
} from '../../../../../src/infrastructure/storage/segments/segment-index-file';
...
describe('SegmentIndexFile', () => {
  ...
  describe('write and load', () => {
    ...
    test('should write and load multiple entries', async () => {
      const entries = [
        createEntry({ streamId: 'stream-a', revision: 0, globalPosition: 0, batchOffset: 100 }),
        createEntry({ streamId: 'stream-a', revision: 1, globalPosition: 1, batchOffset: 100 }),
        createEntry({ streamId: 'stream-b', revision: 0, globalPosition: 2, batchOffset: 200 }),
        createEntry({ streamId: 'stream-c', revision: 0, globalPosition: 3, batchOffset: 300 }),
      ];
    ...
    test('should deduplicate streamIds in string table', async () => {
      ...
      const entries = Array.from({ length: 100 }, (_, i) =>
        createEntry({ streamId: 'same-stream', revision: i, globalPosition: i })
      );
  ...
  describe('findByStream', () => {
    ...
    beforeEach(async () => {
      const entries = [
        createEntry({ streamId: 'stream-a', revision: 0, globalPosition: 0, batchOffset: 100 }),
        createEntry({ streamId: 'stream-a', revision: 1, globalPosition: 1, batchOffset: 100 }),
        createEntry({ streamId: 'stream-a', revision: 2, globalPosition: 2, batchOffset: 200 }),
        createEntry({ streamId: 'stream-b', revision: 0, globalPosition: 3, batchOffset: 200 }),
        createEntry({ streamId: 'stream-b', revision: 1, globalPosition: 4, batchOffset: 300 }),
        createEntry({ streamId: 'stream-c', revision: 0, globalPosition: 5, batchOffset: 400 }),
      ];
  ...
  describe('findByGlobalPosition', () => {
    test('should find entry by global position', async () => {
      const entries = [
        createEntry({ streamId: 'stream-a', globalPosition: 100 }),
        createEntry({ streamId: 'stream-b', globalPosition: 200 }),
        createEntry({ streamId: 'stream-c', globalPosition: 300 }),
      ];
      ...
      const entry = index.findByGlobalPosition(200);
      expect(entry).toBeDefined();
      expect(entry!.streamId).toBe('stream-b');
    });

    test('should return undefined for non-existent position', async () => {
      const entries = [createEntry({ globalPosition: 100 })];

      await SegmentIndexFile.write(fs, '/data/test.idx', 1n, entries);
      const index = new SegmentIndexFile();
      await index.load(fs, '/data/test.idx');

      const entry = index.findByGlobalPosition(999);
      expect(entry).toBeUndefined();
    });
  });

  describe('findBatchOffsetForGlobalPosition', () => {
    test('should return batch offset for first matching global position', async () => {
      const entries = [
        createEntry({ streamId: 'stream-b', globalPosition: 20, batchOffset: 200 }),
        createEntry({ streamId: 'stream-a', globalPosition: 10, batchOffset: 100 }),
        createEntry({ streamId: 'stream-c', globalPosition: 30, batchOffset: 300 }),
        createEntry({ streamId: 'stream-d', globalPosition: 40, batchOffset: 300 }),
      ];
      ...
      expect(index.findBatchOffsetForGlobalPosition(0)).toBe(100);
      expect(index.findBatchOffsetForGlobalPosition(15)).toBe(200);
      expect(index.findBatchOffsetForGlobalPosition(35)).toBe(300);
      expect(index.findBatchOffsetForGlobalPosition(999)).toBeNull();
    });

    test('should return null when no entries exist', async () => {
      await SegmentIndexFile.write(fs, '/data/test.idx', 1n, []);
      const index = new SegmentIndexFile();
      await index.load(fs, '/data/test.idx');

      expect(index.findBatchOffsetForGlobalPosition(1)).toBeNull();
    });
  });
  ...
  describe('edge cases', () => {
    ...
    test('should handle large global positions', async () => {
      const largePos = Number.MAX_SAFE_INTEGER;
