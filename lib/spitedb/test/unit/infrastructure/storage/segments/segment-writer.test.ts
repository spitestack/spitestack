...
describe('SegmentWriter', () => {
  ...
  describe('open', () => {
    test('should create segment file with valid header', async () => {
      ...
      await writer.open('/data/segment.log', 1n, 100);
      ...
      expect(header.basePosition).toBe(100);
    ...
    test('should throw if already open', async () => {
      ...
      await expect(writer.open('/data/other.log', 2n, 0)).rejects.toThrow(
  ...
  describe('resume', () => {
    test('should continue from given offset', async () => {
      ...
      await writer1.open('/data/segment.log', 1n, 0);
  ...
  describe('index file generation', () => {
    test('should create .idx file on close', async () => {
      ...
      await writer.appendBatch([
        createEvent({ streamId: 'stream-a', revision: 0, globalPosition: 0 }),
        createEvent({ streamId: 'stream-a', revision: 1, globalPosition: 1 }),
      ]);
      await writer.sync();
      await writer.close();
    ...
    test('should include all events in .idx file', async () => {
      const { SegmentIndexFile } = await import('../../../../../src/infrastructure/storage/segments/segment-index-file');

      const writer = new SegmentWriter(fs, serializer, compressor);
      await writer.open('/data/segment.log', 42n, 0);
      await writer.appendBatch([
        createEvent({ streamId: 'stream-a', revision: 0, globalPosition: 100 }),
        createEvent({ streamId: 'stream-b', revision: 0, globalPosition: 101 }),
      ]);
      await writer.appendBatch([
        createEvent({ streamId: 'stream-a', revision: 1, globalPosition: 102 }),
      ]);
      await writer.sync();
      await writer.close();
      ...
      expect(streamAEntries[0]!.globalPosition).toBe(100);
      expect(streamAEntries[1]!.globalPosition).toBe(102);
    });
