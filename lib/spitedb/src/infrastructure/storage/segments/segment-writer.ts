 ...
 * await writer.open('/data/segment-0001.log', 1n, 0);
 *
...
import { encodeBatchRecord, BATCH_HEADER_SIZE } from '../batch/batch-record';
 ...
 */
  async open(path: string, segmentId: bigint, basePosition: number): Promise<void> {
    if (this.handle) {
  ...
  async appendBatch(events: StoredEvent[]): Promise<BatchWriteResult> {
    ...
    // Write to file - ensure all bytes are written (handle short writes)
