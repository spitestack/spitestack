...
export interface StreamMetadata {
  ...
  /** Segment IDs containing events for this stream (Set for O(1) lookups) */
  segments: Set<bigint>;
}
 ...
 * - Checking if a segment contains a stream (using Set internally)
 *
...
export class StreamMap {
   ...
   * If segmentId is new for this stream, adds it to segments set.
   *
   * All operations are O(1) thanks to using Set<bigint> for segments.
   *
  ...
  updateStream(streamId: string, revision: number, segmentId: bigint): void {
    ...
    // Add segment (Set.add is O(1) and handles duplicates automatically)
