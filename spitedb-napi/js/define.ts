...
export type ProjectionModule = string & { [__brand]: 'ProjectionModule' };
...
export interface ProjectionDefinition<TSchema extends SchemaDefinition = SchemaDefinition> {
  ...
  apply: (
    ...
    table: ProjectionTable<SchemaToRow<TSchema>>
  ) => void | Promise<void>;
 ...
 */
let __currentDefinition: ProjectionDefinition | null = null;
 ...
 * @internal
 */
export function __getProjectionDefinition(): ProjectionDefinition | null {
 ...
 * ```
 */
export function defineProjection<TSchema extends SchemaDefinition>(
  modulePath: string,
  definition: ProjectionDefinition<TSchema>
): ProjectionModule {
  // Store the definition for the worker to retrieve
  __currentDefinition = definition as ProjectionDefinition;

  // Return the module path as a branded string
  // The path is passed by the user as import.meta.path
  return modulePath as ProjectionModule;
}
